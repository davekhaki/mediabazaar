...
public function viewMinStock(){
  $stmt = $this->connect()->query("SELECT ProductID,ProductName,ProductPrice,Brand,Quantity,MinimumQuantity FROM stock");
//$stmt->execute();
//$stmt->fetchAll();
...
public function viewSchedule(){
  $stmt = $this->connect()->query("SELECT e.FirstName as Name,s.TimeOfDay,s.Day FROM schedule s INNER JOIN employee e ON s.EmployeeID = e.ID ");
//$stmt->execute();
//$stmt->fetchAll();
  foreach ($stmt as $stock) {
    echo "<tr>
